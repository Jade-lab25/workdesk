...
export type DeletedIdsCategory = 'todos' | 'checkInProjects' | 'checkInRecords' | 'timeRecords' | 'achievementLogs' | 'inspirations' | 'shopItems' | 'fdGoals' | 'fdTasks' | 'fdHabits' | 'fdHabitLogs' | 'summaryDocs' | 'summaryIdeas' | 'summaryLogs';

export type DeletedIds = Record<DeletedIdsCategory, string[]>;

const EMPTY_DELETED_IDS: DeletedIds = {
  todos: [], checkInProjects: [], checkInRecords: [], timeRecords: [], achievementLogs: [], inspirations: [], shopItems: [],
  fdGoals: [], fdTasks: [], fdHabits: [], fdHabitLogs: [], summaryDocs: [], summaryIdeas: [], summaryLogs: [],
};

export function getDeletedIds(): DeletedIds {
  try {
    const saved = localStorage.getItem(DELETED_IDS_KEY);
    if (saved) return { ...EMPTY_DELETED_IDS, ...JSON.parse(saved) };
  } catch {}
  return { ...EMPTY_DELETED_IDS };
}
